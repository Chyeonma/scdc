...
public sealed class ServersController(ICommunityService community) : ApiControllerBase
{
    ...
    [HttpGet("{serverId:guid}/channels")] public async Task<ActionResult<IReadOnlyList<ChannelDto>>> Channels(Guid serverId, CancellationToken ct) => FromResult(await community.ListChannelsAsync(User.GetUserId(), serverId, ct));
